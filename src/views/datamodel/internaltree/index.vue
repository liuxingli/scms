<template>
  <div class="internaltree-container">
    <div class="internaltree-title">{{ $t('tree.internaltreetitle') }}</div>
    <Split style="height: 800px;border-style: solid">
      ...
      <SplitArea :size="45" :min-size="300">
        <div class="internaltree-text">{{ $t('tree.nodedetail') }}</div>
        <el-col :span="22">
          <el-form id="treedetail" ref="detailform" :model="detailform" label-width="270px" size="small">
            ...
            <div v-else>
              ...
              <br>
              <div class="internaltree-text">{{ $t('tree.children') }}</div>
...
export default {
  ...
  methods: {
    ...
    refereshnode(node) {
      ...
      const query = { isinternal: true, path: node.data.path, depth: this.operform.subtreedepth }
